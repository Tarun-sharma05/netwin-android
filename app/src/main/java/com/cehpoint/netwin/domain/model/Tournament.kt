...
data class Tournament(
    ...
    val computedStatus: TournamentStatus
        get() {
            ...
            return when {
                now < startDate - 10 * 60 * 1000 -> TournamentStatus.UPCOMING
                now in (startDate - 10 * 60 * 1000) until startDate -> TournamentStatus.STARTS_SOON
                roomCode != null && now >= startDate && now < endDate -> TournamentStatus.ROOM_OPEN
                now in startDate until endDate -> TournamentStatus.ONGOING
                else -> TournamentStatus.COMPLETED
            }
        }
}
