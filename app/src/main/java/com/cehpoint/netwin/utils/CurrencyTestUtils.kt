...
object CurrencyTestUtils {
    ...
    fun testMultiCurrencySupport() {
        ...
        testCurrencies.forEach { currency ->
            ...
            Log.d(
                TAG,
                "  - Minimum amount: ${NGNTransactionUtils.formatAmount(minAmount, currency)}"
            )
        ...
        testCountries.forEach { (country, expectedCurrency) ->
            val actualCurrency = CurrencyUtils.getDefaultCurrencyForCountry(country)
            val isCorrect = actualCurrency == expectedCurrency
            Log.d(
                TAG,
                "  - $country -> $actualCurrency (Expected: $expectedCurrency, Correct: $isCorrect)"
            )
        }
        ...
        val convertedAmount =
            NGNTransactionUtils.convertCurrency(testAmount, "INR", "USD", exchangeRate)
        Log.d(
            TAG,
            "  - $testAmount INR = ${
                NGNTransactionUtils.formatAmount(
                    convertedAmount,
                    "USD"
                )
            } (Rate: $exchangeRate)"
    ...
    fun testNGNCurrencySupport() {
        ...
        Log.d(
            TAG,
            "Nigerian bank account validation - Valid: $isValidAccount, Invalid: $isInvalidAccount"
        )
        ...
        */
        ...
        fun validateWalletCurrency(wallet: Wallet): Boolean {
            val currencyInfo = CurrencyUtils.getCurrencyInfo(wallet.currency)
            ...
            if (wallet.currencySymbol != currencyInfo.symbol) {
                Log.e(
                    TAG,
                    "Currency symbol mismatch. Expected: ${currencyInfo.symbol}, Got: ${wallet.currencySymbol}"
                )
                return false
            }
